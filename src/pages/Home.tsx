...
import { BookOpen, Users, Video, Heart } from "lucide-react";
...
function AnimCard({ href, icon: Icon, title, desc, delay }: { href: string; icon: React.ElementType; title: string; desc: string; delay: number }) {
...
export default function Home() {
  ...
  const [ctaRef,       ctaVisible]       = useScrollAnimation({ animation: "scale-up" } as any);

  return (
    <PageLayout>
      {/* ── Hero ── */}
      <section className="relative h-[420px] md:h-[520px] overflow-hidden" aria-label="Bienvenida">
        <img src={HERO} alt="Interior de la Iglesia Presbiteriana Bíblica en Cuba"
        ...
        <div className="relative container h-full flex flex-col justify-center items-center text-center text-white gap-6 px-4">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold drop-shadow-lg">
            Iglesia Presbiteriana Bíblica
          </h1>
          <p className="text-lg md:text-xl max-w-2xl drop-shadow opacity-90">
      ...
      {/* ── Welcome ── */}
      <section className="py-16 bg-background" aria-labelledby="welcome-h">
      ...
      {/* ── Features ── */}
      <section className="py-16 bg-card" aria-labelledby="features-h">
        <div className="container">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 list-none m-0 p-0" role="list">
            {FEATURES.map((f, i) => <AnimCard key={f.href} {...f} desc={f.desc} delay={i * 80} />)}
          </ul>
        </div>
      </section>

      {/* ── Fe ── */}
      <section className="py-16 bg-background" aria-labelledby="faith-h">
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div ref={faithRef} className={`anim-fade-right${faithVisible ? " visible" : ""}`}>
              <img src={BIBLIA} alt="Biblia abierta sobre una mesa de madera"
      ...
      {/* ── Comunidad ── */}
      <section className="py-16 bg-card" aria-labelledby="community-h">
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            ...
            <div ref={communityRef} className={`anim-fade-left${communityVisible ? " visible" : ""} order-1 md:order-2`}>
              <img src={COMUNIDAD} alt="Miembros de la comunidad de la iglesia reunidos"
                className="rounded-lg shadow-lg w-full h-80 md:h-96 object-cover"
                loading="lazy" decoding="async" />
            </div>
          </div>
      ...
      {/* ── CTA ── */}
      <section className="py-16 bg-primary text-primary-foreground" aria-labelledby="cta-h">
